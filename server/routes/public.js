...
async function activeOffersRaw() {
...
function offerFor(productType, productId, offers) {
  return offers.find(o => o.product_type === productType && Number(o.product_id) === Number(productId)) || null;
}

publicRouter.get('/packages', async (req, res, next) => {
  try {
    const offers = await activeOffersRaw();
    const rows = await all('SELECT * FROM packages WHERE active = 1 ORDER BY display_order, id');
    const out = rows.map(p => {
      ...
      return {
        id: Number(p.id), name: p.name, slug: p.slug, description: p.description,
        price: Number(price), original_price: Number(original), discount_percentage: discount,
        badge: p.badge, features: parseFeatures(p), category: p.category,
        offer: offer ? { id: Number(offer.id), title: offer.title } : null
      };
    });
    res.json(out);
  } catch (err) { next(err); }
...
publicRouter.get('/groups', async (req, res, next) => {
  try {
    const offers = await activeOffersRaw();
    const rows = await all('SELECT * FROM groups_catalog WHERE active = 1 ORDER BY display_order, id');
    const out = rows.map(g => {
      ...
      const discount = original > price ? Math.round(((original - price) / original) * 100) : 0;
      return {
        id: Number(g.id), name: g.name, description: g.description, category: g.category,
        price: Number(price), original_price: Number(original), discount_percentage: discount,
...
publicRouter.get('/offers', async (req, res, next) => {
  try {
    const offers = await activeOffersRaw();
    const packages = new Map((await all('SELECT id, name FROM packages')).map(p => [Number(p.id), p.name]));
    const groups = new Map((await all('SELECT id, name FROM groups_catalog')).map(g => [Number(g.id), g.name]));
    const out = offers.map(o => ({
      id: Number(o.id), title: o.title, description: o.description,
      product_type: o.product_type, product_id: Number(o.product_id),
      product_name: o.product_type === 'package' ? packages.get(Number(o.product_id)) : groups.get(Number(o.product_id)),
      original_price: Number(o.original_price), sale_price: Number(o.sale_price),
...
publicRouter.get('/faq', async (req, res, next) => {
  try {
    const rows = await all('SELECT id, question, answer FROM faqs WHERE active = 1 ORDER BY display_order, id');
    res.json(rows);
  } catch (err) { next(err); }
});
...
publicRouter.post('/orders', async (req, res, next) => {
  try {
    ...
    if (product_type === 'package') {
      product = await get('SELECT * FROM packages WHERE id = ? AND active = 1', [Number(product_id)]);
      ...
      product = await get('SELECT * FROM groups_catalog WHERE id = ? AND active = 1', [Number(product_id)]);
    ...
    await run(
      `INSERT INTO orders (order_id, customer_name, customer_contact, product_type, product_id, product_name, amount, payment_reference, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
      [orderId, String(customer_name).trim().slice(0, 120), String(customer_contact).trim().slice(0, 160),
        product_type, product.id, product.name, price, String(payment_reference || '').trim().slice(0, 120)]
    );

    res.status(201).json({ order_id: orderId, amount: Number(price), status: 'pending' });
  } catch (err) { next(err); }
});
